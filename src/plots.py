...
def plot_sentiment_count(df: pd.DataFrame):
    ...
    negative_tweets = df[df.sentiment == 'Negative'].groupby(df.created_at)['created_at'].count().rename('count').reset_index()
    positive_tweets = df[df.sentiment == 'Positive'].groupby(df.created_at)['created_at'].count().rename('count').reset_index()
    neutral_tweets = df[df.sentiment == 'Neutral'].groupby(df.created_at)['created_at'].count().rename('count').reset_index()
